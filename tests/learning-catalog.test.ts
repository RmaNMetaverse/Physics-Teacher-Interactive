...
function assessment(id: string): Assessment {
  return { id, kind: 'concept', prompt: 'Which value is positive?', options: ['-1', '1'], answer: 1, hints: ['Compare each value with zero.'], explanation: 'One is greater than zero, so it is positive.' };
}
function mathLayer(checkId: string): MathLayer {
  return {
...
function mission(id = 'motion-start'): MissionDefinition {
  return {
    ...
    steps: [
      ...
      { id: 'math-motion', kind: 'math', title: 'Speed', layer: mathLayer(`${id}-math-check`) },
      { id: 'explain-motion', kind: 'explain', title: 'Explain', body: ['Speed compares the observed distance with elapsed time.'] },
      { id: 'check-motion', kind: 'check', assessment: assessment(`${id}-check`) },
      { id: 'recap-motion', kind: 'recap', takeaways: ['Speed compares distance and time.'] },
    ],
    sources: [{ label: 'OpenStax University Physics', url: 'https://openstax.org/details/books/university-physics-volume-1' }], limitations: ['This model represents one-dimensional motion.'],
  };
}
function checkpoint(requiredMissionIds = ['motion-start'], id = 'motion-checkpoint'): MissionDefinition {
  return {
    ...
    sources: [{ label: 'OpenStax University Physics', url: 'https://openstax.org/details/books/university-physics-volume-1' }], limitations: ['This checkpoint samples only the ideas in this starter mission.'],
  };
}
function course(id = 'quantum', missionId = 'motion-start', recommendations: string[] = []): CourseDefinition {
  const normal = mission(missionId);
  return {
    id, title: 'Quantum Physics', description: 'A starter path for thinking about quantum physics.', group: 'modern', scope: 'Starter path', color: '#6d5efc', recommendations, access: 'open', estimatedMinutes: 10,
    missions: [normal, checkpoint([missionId], `${id}-checkpoint`)],
...
describe('course catalog validation', () => {
  ...
  it('rejects duplicate mission IDs across a catalog', () => {
    expect(() => createCourseCatalog([course(), course('quantum-two', 'motion-start')])).toThrow(/duplicate mission/i);
  });
  ...
  it('rejects a math step without the expandable foundation layer', () => {
    ...
    const math = steps[mathIndex];
    if (!math || math.kind !== 'math') throw new Error('Missing math fixture');
    steps[mathIndex] = { ...math, layer: { quick: math.layer.quick } as MathLayer };
  ...
  it('rejects recommendation cycles while leaving recommendations non-blocking', () => {
    expect(() => createCourseCatalog([course('quantum', 'quantum-start', ['relativity']), course('relativity', 'relativity-start', ['quantum'])])).toThrow(/recommendation cycle/i);
  });

...
function validateAssessment(assessment: Assessment | undefined, context: string): void {
  if (!assessment || typeof assessment !== 'object') fail(`${context} requires an assessment`);
  requireText(assessment.id, `${context} assessment ID`);
...
function validateMathLayer(layer: MathLayer | undefined, context: string): void {
  ...
  requireFinitePositive(visual.min, `${context} expanded math visual minimum`, true); requireFinitePositive(visual.max, `${context} expanded math visual maximum`, true); requireFinitePositive(visual.step, `${context} expanded math visual step`); requireFinitePositive(visual.initial, `${context} expanded math visual initial`, true);
  ...
  requireText(visual.instruction, `${context} expanded math visual instruction`); requireText(foundation.workedExample?.question, `${context} expanded math worked example question`); requireTextList(foundation.workedExample?.steps, `${context} expanded math worked example steps`); requireText(foundation.workedExample?.answer, `${context} expanded math worked example answer`); validateAssessment(foundation.check, `${context} expanded math`);
}
...
function validateSteps(mission: MissionDefinition): void {
  if (!Array.isArray(mission.steps) || mission.steps.length === 0) fail(`mission ${mission.id} must contain steps`);
  const stepIds = new Set<string>(); let recapCount = 0; let hasLearnerAction = false;
  mission.steps.forEach((step: MissionStep, index) => {
    ...
    stepIds.add(step.id);
    switch (step.kind) {
      case 'observe': case 'explain': requireText(step.title, `mission ${mission.id} step ${step.id} title`); requireTextList(step.body, `mission ${mission.id} step ${step.id} body`); break;
      case 'predict': case 'check': validateAssessment(step.assessment, `mission ${mission.id} step ${step.id}`); hasLearnerAction = true; break;
      ...
      case 'math': requireText(step.title, `mission ${mission.id} step ${step.id} title`); validateMathLayer(step.layer, `mission ${mission.id} step ${step.id}`); break;
...
function validateMission(mission: MissionDefinition, normalMissionIds: Set<string>): void {
  ...
  validateSteps(mission); validateSources(mission.sources, `mission ${mission.id}`); requireTextList(mission.limitations, `mission ${mission.id} limitations`); validateCheckpoint(mission.checkpoint, mission, normalMissionIds);
}
...
export function createCourseCatalog(courses: readonly CourseDefinition[]): CourseCatalog {
  ...
  const courseMap = new Map<string, CourseDefinition>(), missionMap = new Map<string, MissionDefinition>();
  for (const course of courses) {
    ...
    for (const mission of missions) { if (missionMap.has(mission.id)) fail(`duplicate mission ID ${mission.id}`); validateMission(mission, normalMissionIds); if (mission.modelId !== undefined || mission.steps.some((step: MissionStep) => step.kind === 'simulate')) hasSimulation = true; missionMap.set(mission.id, mission); }

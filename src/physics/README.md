...
| `circuits` | `mode` 0–3; `voltage` V [0, 12] (5); `resistance`, `resistance2` Ω [100, 10000] (330, 1000); `switchClosed` 0/1; `ledDrop` V [1, 3.5] (2) | Ideal DC resistor, series, parallel and fixed-drop LED templates. `totalCurrent` A; `equivalentResistance` Ω; branch currents A; resistor voltage V; supply power W. |
| `microcontroller` | `mode` 0–3; `board` UNO/ESP32; `period` s [.2, 5] (2); `buttonPressed` 0/1; `duty` % [0, 100] (50); `analogVoltage` V [0, 5] (2.5); `resistance` Ω [100, 2200] (330) | Predefined Blink, active-high button, ideal PWM, and ADC behaviors. UNO uses 5 V/10-bit and original ESP32 uses 3.3 V/12-bit reference presets. Output levels and ideal ADC codes are dimensionless; estimated LED current is A. |
...
- **Circuits:** four fixed ideal DC topologies, not arbitrary connectivity or a circuit design simulator. The LED uses a clamped fixed forward drop; no tolerance, reverse bias, capacitance, transistor switching, meter loading or thermal/current-limit simulation. Browser schematic and breadboard views render the same model values and have no WebGL cost.
- **Microcontroller:** prescribed output/button/PWM/ADC behaviors, not a compiler, firmware emulator or verified physical pinout. The illustrative original ESP32 ADC scaling omits calibration and attenuation; an over-range input is clipped only in software visualization and is unsafe on real hardware.

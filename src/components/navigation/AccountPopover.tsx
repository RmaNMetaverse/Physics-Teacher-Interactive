...
interface AccountPopoverProps {
  ...
  isOpen: boolean;
  onClose(): void;
  variant?: 'popover' | 'gate';
}

export function AccountPopover({ account, isOpen, onClose, variant = 'popover' }: AccountPopoverProps) {
  ...
  useEffect(() => {
    if (isOpen && isGate) heading.current?.focus();
  }, [isOpen, isGate]);
  ...
  function trapFocus(event: KeyboardEvent<HTMLElement>) {
    if (!isGate || event.key !== 'Tab') return;
  ...
  return (
    <section
      ref={dialog}
      className={`account-popover ${isGate ? 'account-popover-gate' : ''}`}
      role="dialog"
      aria-label={isGate ? 'Sign in to sync progress' : 'Account and cloud sync'}
      aria-modal={isGate ? true : undefined}
      onKeyDown={trapFocus}
    >
      <header className="account-popover-header">
        <div>
          <span className="account-eyebrow">{isGate ? 'Save your learning' : 'Cloud account'}</span>
          <h2 ref={heading} tabIndex={isGate ? -1 : undefined}>
            {account.user ? 'Your progress is protected' : isGate ? 'Keep your physics progress everywhere' : 'Continue on any device'}
          </h2>
        </div>
        <button type="button" className="appearance-popover-close" aria-label={isGate ? 'Continue without signing in' : 'Close account'} onClick={onClose}>
          <X size={16} aria-hidden="true" />
        </button>
      </header>

      {isGate && !account.user && (
        <p className="account-gate-intro">Sign in to protect your achievements, streak, and course progress and continue learning on any device.</p>
      )}
      ...
      {!account.configured ? (
        ...
        <>
          ...
          <div className="account-sso-buttons">
            <button type="button" className="account-sso-button" onClick={() => void signInWithProvider('google')} disabled={busy}>
            ...
            <button type="button" className="account-sso-button" onClick={() => void signInWithProvider('github')} disabled={busy}>
